...
void DDM<T>::computeDDM() {
    ...
    this->raw_ddm_buffer = fftwf_alloc_real(delays.getIndex().size() * rawDdmSize());
...
void DDM<T>::save() {
    ...
    for(int frame = 0; frame < delays.getIndex().size(); frame++) {
        float* data = &ddm_buffer[frame * this->ddmSize()];
...
void DDM<T>::ddmshift() {
    ...
    this->ddm_buffer = fftwf_alloc_real(delays.getIndex().size() * ddmSize());
    ...
    auto raw_idx = [&](int i, int j, int k) -> int {
        return i * rawDdmSize() + j * raw_ddm_width + k;
    };
...
void DDM<T>::ddm_loop_autovec_delays() {
     ...
     */
    // init
    for(int i = 0; i < rawDdmSize() * delays.getIndex().size(); i++)
        raw_ddm_buffer[i] = 0;
