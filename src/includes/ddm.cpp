...
#pragma omp parallel for schedule(nonmonotonic:dynamic)
    for(int idx_delay = 0; idx_delay < delays.getIndex().size(); idx_delay++) {
        ...
        for(t = delays.getIndex().back(); t < options.loadNframes; t++) {
            i1 = static_cast<const float*>(&stack_fft[t*raw_ddm_size][0]);
            i2 = static_cast<const float*>(&stack_fft[(t-delays.getIndex()[idx_delay])*raw_ddm_size][0]);
            for(pix=0; pix<raw_ddm_size; pix+=4) {
...
void DDM<T>::ddm_loop_autovec_delays() {
	 ...
	 * and Delays information.
     *
	 * This function computes the DDM differences based on the FFT results, utilizing
	 * autovectorization for parallelization. It performs the following steps:
     *   1. Initializes the raw DDM buffer to zeros.
...
#pragma omp parallel for schedule(nonmonotonic:dynamic)
    for(int idx_delay = 0; idx_delay < delays.getIndex().size(); idx_delay++) {
        ...
        for(t = delays.getIndex().back(); t < options.loadNframes; t++) {
            i1 = &stack_fft[t*raw_ddm_size];
            i2 = &stack_fft[(t-delays.getIndex()[idx_delay])*raw_ddm_size];
