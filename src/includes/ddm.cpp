...
template<typename T>
DDM<T>::DDM(
		Stack& stack, Delays<T>& delays, utils::Options& options) :
	 ...
	 * @param delays Reference to the Delays structure specifying the time delays for DDM
	 *        computation.
...
#ifdef __AVX512F__
    std::cout << "* Computing DDM differences with AVX512..." << std::flush;
    this->raw_ddm_buffer = (float*) _mm_malloc(delays.getIndex().size()*raw_ddm_size*sizeof(float), 64);
    ddm_loop_avx512();
#elif __AVX2__
    std::cout << "* Computing DDM differences with AVX2..." << std::flush;
    this->raw_ddm_buffer = fftwf_alloc_real(delays.getIndex().size()*raw_ddm_size);
    ddm_loop_avx2();
#else
    std::cout << "* Computing DDM differences with autovec..." << std::flush;
    this->raw_ddm_buffer = fftwf_alloc_real(delays.getIndex().size()*raw_ddm_size);
    ddm_loop_autovec();
#endif
...
void DDM<T>::ddm_loop_avx512() {
    ...
    for(int idx_delay = 0; idx_delay < delays.getIndex().size(); idx_delay++) {
        ...
        float* ddm_cur = &raw_ddm_buffer[idx_delay * raw_ddm_size];
        ...
        for(it = delays.getIndex().back(); it < options.loadNframes; it++) {
            i1 = static_cast<const float*>(&stack_fft[it*raw_ddm_size][0]);
            i2 = static_cast<const float*>(&stack_fft[(it-delays.getIndex()[idx_delay])*raw_ddm_size][0]);
            for(idx_buf=0; idx_buf<2*raw_ddm_size; idx_buf+=16) {
                m512_a = _mm512_load_ps(&i1[idx_buf]);
                m512_b = _mm512_load_ps(&i2[idx_buf]);
                m512_c = _mm512_sub_ps(m512_a, m512_b);
                m512_a = _mm512_permutexvar_ps(perm, m512_c);

                m256_a = _mm256_load_ps(&ddm_cur[idx_buf/2]);
                ...
                _mm256_store_ps(&ddm_cur[idx_buf/2], m256_b);
            }
        }
    }

    float mean_weight = 1. / ( 2*raw_ddm_size * (options.loadNframes-delays.getIndex().size()) );
    for(int i=0; i<raw_ddm_size*delays.getIndex().size(); i++)
...
template <typename T>
void DDM<T>::ddm_loop_avx2() {
    /**
     * Comment generated by GPT-3
     *
     * Perform the DDM (Differential Dynamic Microscopy) computation using AVX2
     * instructions.
     *
     * This function computes the DDM differences based on the FFT results, utilizing AVX2
     * instructions for vectorization. It performs the following steps:
     *   1. Initializes the raw DDM buffer to zeros.
    ...
    for(int idx_delay = 0; idx_delay < delays.getIndex().size(); idx_delay++) {
        ...
        for(it = delays.getIndex().back(); it < options.loadNframes; it++) {
            i1 = static_cast<const float*>(&stack_fft[it*raw_ddm_size][0]);
            i2 = static_cast<const float*>(&stack_fft[(it-delays.getIndex()[idx_delay])*raw_ddm_size][0]);
            for(idx_buf=0; idx_buf<2*raw_ddm_size; idx_buf+=8) {
                avx_a = _mm256_load_ps(&i1[idx_buf]);
                avx_b = _mm256_load_ps(&i2[idx_buf]);
                avx_c = _mm256_sub_ps(avx_a, avx_b);
                avx_a = _mm256_permutevar8x32_ps(avx_c, perm);

                sse_a = _mm_load_ps(&ddm_cur[idx_buf/2]);
                ...
                _mm_store_ps(&ddm_cur[idx_buf/2], sse_b);
            }
        }
    }
...
template <typename T>
void DDM<T>::ddm_loop_autovec() {
    /**
     * Comment generated by GPT-3
     *
	 * Perform the DDM (Differential Dynamic Microscopy) computation using autovectorization.
     *
	 * This function computes the DDM differences based on the FFT results, utilizing
	 * compiler autovectorization for parallelization. It performs the following steps:
     *   1. Initializes the raw DDM buffer to zeros.
...
#pragma omp parallel for schedule(nonmonotonic:dynamic)
    for(int idx_delay = 0; idx_delay < delays.getIndex().size(); idx_delay++) {
        ...
        for(it = delays.getIndex().back(); it < options.loadNframes; it++) {
            ...
            i2 = &stack_fft[(it-delays.getIndex()[idx_delay])*raw_ddm_size];
    ...
    float mean_weight =
		1. / ( 2*raw_ddm_size * (options.loadNframes-delays.getIndex().size()) );
    for(int i=0; i<raw_ddm_size*delays.getIndex().size(); i++)
        raw_ddm_buffer[i] *= mean_weight;
}
...
void DDM<T>::save() {
   ...
   //  TinyTIFFWriterFile* tif = TinyTIFFWriter_open(
			// options.pathOutput.c_str(), 32, TinyTIFFWriter_Float,
			// 1, ddm_width, ddm_height, TinyTIFFWriter_Greyscale);
	TIFF* tif = TIFFOpen((this->options.pathOutput+"_ddm.tif").c_str(), "w");
    if(!tif)
        throw std::runtime_error("Can't write files");

    for(int frame = 0; frame < delays.getIndex().size(); frame++)
        // TinyTIFFWriter_writeImage(tif, &ddm_buffer[frame*this->ddmSize()]);
		utils::libTIFFWriter_writeImage(
				tif, &ddm_buffer[frame*ddm_size], ddm_width, ddm_height);
