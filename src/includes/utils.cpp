...
template<typename T>
std::vector<T> utils::linspace(double start, double end, int num_points) {
    std::vector<T> result;
...
template<typename T>
std::vector<T> utils::logspace(double start, double end, int num_points) {
    std::vector<T> result;
    double step = (end - start) / (num_points - 1);
    for (int i = 0; i < num_points; i++) {
...
template<typename T>
utils::lspace<T>* utils::delays(float sampling_time, utils::Options& opt, std::string mode) {
    ...
    std::transform(space->time.begin(), space->time.end(), space->index.begin(), [sampling_time](T x) -> int {
        //std::cout << "=>" << x << " " << std::ceil(x/sampling_time)<< std::endl;
